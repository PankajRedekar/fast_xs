...
static inline size_t bytes_for(int n)
{
    ...
    if (n < 1000000)
        return sizeof("&#999999;") - 1;
    /* we don't expect n >= 10000000 since Unicode max is 0x10FFFF */
...
/* Wrapper to match signature required by rb_rescue for rescue callback */
static VALUE unpack_uchar_rescue(VALUE arg, VALUE err)
{
    (void)err;  /* suppress unused parameter warning */
    return unpack_uchar(arg);
}
...
static VALUE fast_xs(VALUE self)
{
    ...
    /* Try unpack("U*"), fallback to unpack("C*") */
    array = rb_rescue(unpack_utf8, self, unpack_uchar_rescue, self);

    ary_len = RARRAY_LEN(array);
    s_len = (size_t)ary_len;  /* minimal size, each element at least 1 byte */

    /* first pass: estimate size needed */
    for (i = 0; i < ary_len; ++i) {
    ...
    /* second pass: write out escaped data */
    for (i = 0; i < ary_len; ++i) {
        VALUE elem = rb_ary_entry(array, i);
        int n = NUM2INT(elem);
        c += escape(c, n);
...
{
    assert(cp_1252[159 - 128] == 376);

    unpack_id = rb_intern("unpack");

    U_fmt = rb_str_new_cstr("U*");
    C_fmt = rb_str_new_cstr("C*");

    /* ensure these strings are GC-protected */
